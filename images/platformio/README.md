...
versioning expects a bare `<major>.<minor>.<patch>`, not `pio-v<version>`, so every tag here
  ...
  "packageRules": [
    {
      ...
      "matchPackageNames": ["ghcr.io/jethome-iot/jethome-dev-platformio"],
      "versioning": "regex:^pio-v(?<major>\\d+)\\.(?<minor>\\d+)\\.(?<patch>\\d+)$",
      "pinDigests": true
    }
  ]
}
